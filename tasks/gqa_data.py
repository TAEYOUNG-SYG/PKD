...
class GQADataset:
    ...
    def __init__(self, file_name: str):
        ...
        self.data = []
        self.data.extend(json.load(open(self.name, 'r')))
        print("Load data from %s" % (self.name))
        ...
        # Answers
        self.ans2label = json.load(open("/hdd/ygsong/D-VQA/gqa/questions/trainval_ans2label.json"))
        self.label2ans = json.load(open("/hdd/ygsong/D-VQA/gqa/questions/trainval_label2ans.json"))
...
class GQABufferLoader():
    ...
    def load_data(self, name, number):
        if name == '/hdd/ygsong/D-VQA/gqa/questions/ood/processed/testdev_all.json':
            path = "/hdd/ygsong/D-VQA/gqa/feat/vg_gqa_imgfeat/gqa_testdev_obj36.tsv"
        else:
            path = "/hdd/ygsong/D-VQA/gqa/feat/vg_gqa_imgfeat/vg_gqa_obj36.tsv"
...
class GQATorchDataset(Dataset):
    def __init__(self, dataset: GQADataset):
        ...
        img_data = []
        if 'test' in dataset.name:  # Always loading all the data in testdev
            img_data.extend(gqa_buffer_loader.load_data(dataset.name, -1))
        else:
            img_data.extend(gqa_buffer_loader.load_data(dataset.name, topk))
        self.imgid2img = {}
